function Hofstadter_triangle_2site(t,Lx,Ly,P,M,kxs,kys,W)
    N=Lx*Ly;
    ...
    for ca=1:Lx
        for cb=1:Ly
            ...
            hAA=2*t*cos(ky);
            hAB=t*(exp(im*phi)+exp(-im*phi)*exp(im*kx)-exp(im*ky)+exp(-im*ky+im*kx));
            hk=[hAA hAB;hAB' -hAA];
...
function Qi_Wu_Zhang_modified(ta,tb,tc,td,Mz,Lx,Ly,P,M,kxs,kys,W)
    N=Lx*Ly;
    ...
    for ca=1:Lx
        for cb=1:Ly
            ...
            hk=ta*2*sin(ky)*sigmax-tb*2*sin(kx)*sigmay+(Mz-tc*2*cos(kx)-td*2*cos(ky))*sigmaz;
            E=E+real(tr(hk*rho_out));
            #println(eigvals(C_out))
        end
    end
